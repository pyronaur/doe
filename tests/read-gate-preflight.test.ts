...
const PI_ROOT = process.env.PI_CODING_AGENT_ROOT ?? "/opt/homebrew/lib/node_modules/@mariozechner/pi-coding-agent";
const PI_AGENT_CORE_ROOT = process.env.PI_AGENT_CORE_ROOT ?? join(PI_ROOT, "node_modules/@mariozechner/pi-agent-core");
...
async function loadPiModules() {
	...
	piModulesPromise = (async () => {
		...
		const [{ createExtensionRuntime }, { ExtensionRunner }, { agentLoop }, { createAssistantMessageEventStream }] = await Promise.all([
...
function createExtension(path: string, handler: (event: any, ctx: any) => Promise<any> | any) {
	return {
		path,
...
function createAssistantMessage(content: any[], stopReason: "toolUse" | "stop" | "error" | "aborted") {
	return {
		role: "assistant",
		content,
		api: "test-api",
...
if (!IS_BUN) test("ExtensionRunner emitToolCall stops at the first immediate toolResult", async () => {
...
if (!IS_BUN) test("agentLoop treats an immediate preflight toolResult as nonfatal and continues the turn", async () => {
	...
	const messages = await agentLoop(
			...
			tools: [
				{
					...
					parameters: READ_TOOL_SCHEMA as any,
					execute: async () => {
						toolExecutions += 1;
						return {
		...
		{
			model: { api: "test-api", provider: "test-provider", id: "test-model" } as any,
		...
		async () => {
			...
			return await createResponse(createAssistantMessage([{ type: "text", text: "I will choose another route." }], "stop"));
		},
	).result();
	...
	assert.deepEqual(finalAssistant?.content, [{ type: "text", text: "I will choose another route." }]);
});

if (!IS_BUN) test("agentLoop still treats blocked preflight results as errors", async () => {
	const { agentLoop } = await loadPiModules();
	let toolExecutions = 0;
	let streamCalls = 0;
	const messages = await agentLoop(
		...
		async () => {
			...
			return await createResponse(createAssistantMessage([{ type: "text", text: "Stopping after the block." }], "stop"));
		},
	).result();
