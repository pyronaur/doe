...
interface FindNestedValueOptions {
	depth?: number;
	seen?: Set<unknown>;
}
...
function findNestedValue<T>(
	...
	options: FindNestedValueOptions = {},
): T | null {
	const { depth = 0, seen = new Set<unknown>() } = options;
	...
	if (Array.isArray(value)) {
		for (const entry of value) {
			const found = findNestedValue(entry, predicate, { depth: depth + 1, seen });
			if (found !== null) {
				return found;
			}
		}
...
function findNestedString(value: unknown, keys: string[]): string | null {
	...
	return findNestedValue(
		...
		(current, key): current is string =>
			typeof current === "string" && current.trim().length > 0 && key !== null
			&& keySet.has(key.toLowerCase()),
	);
}
...
function findNestedNumber(value: unknown, keys: string[]): number | null {
	...
	return findNestedValue(
		value,
		(current, key): current is number =>
			typeof current === "number" && Number.isFinite(current) && key !== null
			&& keySet.has(key.toLowerCase()),
	);
}

function getThreadItemText(item: any): string {
...
function getThreadItemPath(item: any): string | null {
	...
	const oldPath = typeof item?.oldPath === "string" && item.oldPath.trim()
		? item.oldPath.trim()
		: null;
	const newPath = typeof item?.newPath === "string" && item.newPath.trim()
		? item.newPath.trim()
		: null;
	if (oldPath && newPath) {
		return oldPath === newPath ? newPath : `${oldPath} -> ${newPath}`;
	}
	return findNestedString(item, [
		"path",
		"filePath",
		"targetPath",
		"destinationPath",
...
function getThreadItemDiffStats(item: any): { addedLines: number | null; removedLines: number | null } {
	...
	const removedLines = findNestedNumber(item, [
		"removedLines",
		"linesRemoved",
		"removed",
		"deletions",
...
function summarizeThreadItem(item: any): string {
	...
	if (item?.type === "fileChange") {
		...
		const stats = addedLines !== null || removedLines !== null
...
export function extractThreadTranscript(
	...
	for (const turn of thread?.turns ?? []) {
		for (const item of turn.items ?? []) {
			...
			const completedAt = item?.type === "agentMessage"
				? parseTimestamp(item?.completedAt, createdAt)
				: createdAt;
			messages.push({
				turnId: turn.id,
...
export function extractThreadMessages(
	thread: ThreadSummary | null | undefined,
): ThreadMessageEntry[] {
...
export function extractLastCompletedAgentMessage(
	thread: ThreadSummary | null | undefined,
): string | null {
...
export function extractThreadFileChanges(
	thread: ThreadSummary | null | undefined,
	...
	for (const turn of thread?.turns ?? []) {
		for (const item of turn.items ?? []) {
			...
			existing.changes += 1;
			if (!existing.turnIds.includes(turn.id)) {
				existing.turnIds.push(turn.id);
			}
			if (addedLines !== null) {
				existing.addedLines = (existing.addedLines ?? 0) + addedLines;
			}
			if (removedLines !== null) {
				existing.removedLines = (existing.removedLines ?? 0) + removedLines;
			}
		}
	}
...
export function extractThreadQueryEntries(
	thread: ThreadSummary | null | undefined,
): ThreadQueryEntry[] {
