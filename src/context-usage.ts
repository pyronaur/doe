...
function normalizeEffectiveUsage(input: ThreadTokenUsage | CurrentContextUsage): CurrentContextUsage | null {
	...
	const fallbackTotal =
		(input.last?.inputTokens ?? 0) + (input.last?.outputTokens ?? 0) + (input.last?.reasoningOutputTokens ?? 0);
	const rawTokensUsed =
		typeof lastTotal === "number" && Number.isFinite(lastTotal) && lastTotal > 0
			? lastTotal
...
export function deriveUsageSnapshot(tokenUsage: ThreadTokenUsage | CurrentContextUsage, turnId: string | null, updatedAt = Date.now()): AgentUsageSnapshot {
	...
	const usedPercent = tokenLimit && tokenLimit > 0 && tokensUsed !== null
		...
		: null;
	return {
		total: "total" in tokenUsage ? tokenUsage.total : { totalTokens: 0, inputTokens: 0, cachedInputTokens: 0, outputTokens: 0, reasoningOutputTokens: 0 },
		last: "last" in tokenUsage ? tokenUsage.last : { totalTokens: tokensUsed ?? 0, inputTokens: 0, cachedInputTokens: 0, outputTokens: 0, reasoningOutputTokens: 0 },
		modelContextWindow: "modelContextWindow" in tokenUsage ? tokenUsage.modelContextWindow : tokenLimit,
		turnId,
		tokensUsed,
		tokenLimit,
		remainingTokens: tokenLimit !== null && tokensUsed !== null ? Math.max(0, tokenLimit - tokensUsed) : null,
		usedPercent,
...
export function formatUsageCompact(
	...
	if (!snapshot || snapshot.usedPercent === null || snapshot.tokensUsed === null) return "ctx n/a";
	return `${snapshot.usedPercent}% (${formatCompactCount(snapshot.tokensUsed)})`;
}

export function formatCompactionSignal(compaction: AgentCompactionState | null | undefined): string | null {
...
export function formatUsageBreakdown(
	...
	if (!snapshot) {
		...
		lines.push(
			`usage snapshot: used=${formatCount(snapshot.tokensUsed)} limit=${formatCount(snapshot.tokenLimit)} remaining=${formatCount(snapshot.remainingTokens)} turn=${snapshot.turnId ?? "?"}`,
		);
	}
	if (!compaction) return lines;
	if (compaction.inProgress) {
		lines.push(`compaction: in progress${compaction.lastTurnId ? ` on turn ${compaction.lastTurnId}` : ""}`);
		return lines;
	}
	if (compaction.count <= 0) return lines;
	const freshness = usageFreshness(snapshot, compaction);
	const status =
		freshness === "post_compaction"
	...
	lines.push(`compaction: completed ${compaction.count}x${compaction.lastTurnId ? ` on turn ${compaction.lastTurnId}` : ""}; ${status}`);
